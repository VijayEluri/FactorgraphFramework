...
public class Graph {
	 ...
	 */
	public int getDefSize(Message m) { // gibt die Größe der Definitionsmenge
										// einer Message zurück
		int size = 0;
	...
	public Hashtable<Node, Hashtable<Node, Hashtable<Object, Double>>> getFinalHash() {
		...
		for (int i = 0; i < mSeq.size(); i++) { // Schleife für fromHash
			...
			for (int j = 0; j < keyFrom.length; j++) { // hier wird getestet ob
														// der From-Wert bereits
														// im Hash existiert
			...
			if (!fromIsInHash) {
				for (int j = 0; j < mSeq.size(); j++) { // Schleife für toHash
					...
					if (mi.getFrom().equals(mj.getFrom())) { // findet alle
																// Kanten die
																// den gleichen
																// FromKnoten
																// haben
						...
						for (int x = 0; x < getDefSize(message); x++) { // Schleife
																		// für
																		// ValueHash

							if (message.isVariableToFactor()) {

								VariableNode variable = (VariableNode) mi
										.getFrom();
						...
						toHash.put(mSeq.get(j).getTo(),
								new Hashtable<Object, Double>(valueHash));
						valueHash.clear();
					}

				}
				fromHash.put(mSeq.get(i).getFrom(),
						new Hashtable<Node, Hashtable<Object, Double>>(toHash));
				toHash.clear();
			}
	...
	public Message getMessage(Node from, Node to) { // gibt die Message zurück,
													// wenn man den From-Knoten
													// und den To-Knoten hat
		...
		System.out
				.println("Es gibt keine Message mit diesen From und To Knoten");
		return null;
	}
	...
	public void print() {
		...
		for (int i = 0; i < keyFrom.length; i++) {
			...
			for (int x = 0; x < keyTo.length; x++) {
				...
				Object keyValue[] = finalHash.get(knoten).get(toKnoten)
						.keySet().toArray();
				for (int j = 0; j < keyValue.length; j++) {
					double value = finalHash.get(knoten).get(toKnoten).get(
							keyValue[j]);
					System.out.println("From: " + knoten.getName() + " To: "
							+ toKnoten.getName() + " für Def. " + keyValue[j]
							+ " Value: " + value);
				}
