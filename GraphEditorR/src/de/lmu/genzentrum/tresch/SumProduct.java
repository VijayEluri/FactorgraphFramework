...
public class SumProduct {
	...
	protected void calcFactorToVariable(int index) throws NoValueException {
		...
		if (toHash.size() <= 1) { // nur für Kanten die aus Blättern kommen
				...
				if (toNode == to) {
					...
					for (int j = 0; j < this.graph.getDefSize(m); j++) {
						toHash.get(toNode).put(variable.getValues()[j],
								(Double) function[1][j]); // man fügt zum
															// valueHash für
															// jedes Def die
															// Funktionswerte
															// hinzu;
			...
			for (int y = 0; y < function[0].length; y++) { // die Tabelle wird
				...
				double res = 0;
				if (!result.containsKey(function[i][y])) { // falls für den
															// funktionswert
															// bereits ein
															// eitrag existiet
					res = wert;
			...
			if (finalArray[i].getClass().getName().equals("VariableNode")) { // man
				...
				for (int t = 0; t < wert.length; t++) {
					wert[t] = 1; // man muss das wert-array mit dem neutralen
									// Element der Multiplikation füllen
				}
				...
				for (int t = 0; t < wert.length; t++) {
					...
					System.out.println("Node " + node.getName()
							+ " hat bei Def " + def + " den Wert " + wert[t]); // ränder
																				// werden
																				// ausgegeben
				}
			}
	...
	protected Node[] getFromNodes(Message m) { // man bekommt die Nodes die über
											// eine Message in den fromNode von
											// m führen und nicht toNode von m
											// sind
